...
/*
Loại bỏ dấu tiếng Việt trong chuỗi nhập vào
*/
...
/*---------------------------------
Kiểm tra coi có phải premium không
-----------------------------------*/
function isPremium() {
...
/*---------------------------------
Kiểm tra coi có phải genomic không
-----------------------------------*/
function isGenomic() {
...
/*---------------------------------
Kiểm tra coi có phải probe không
-----------------------------------*/
function isProbe() {
    return (productSku.innerText == 'Probe') ? true : false;
}
/*---------------------------------
Kiểm tra coi có phải single không
-----------------------------------*/
function isSingle() {
...
/*---------------------------------
Hàm thêm cột vị trí cho danh sách oligo
-----------------------------------*/
...
function summaryOligo() {
    ...
    //     <td scope="col" class="text-end pe-5">${totalAmount)}đ</td>
    // </tr>`;
    packageBoxFee += (totalAmount <= minOrderValue) ? packageFee : 0;
    sumText += (packageBoxFee != 0) ? `<tr><td scope="col" colspan="11" class="text-start ps-5">${packageFeeText + currencyDisplay(minOrderValue)}đ</td><td scope="col" colspan="2" class="text-end pe-5">${currencyDisplay(packageBoxFee)}đ</td></tr>` : ``;
    sumText += (plateArrangeFee != 0) ? `<tr><td scope="col" colspan="11" class="text-start ps-5">${basePlateArrangeFeeText}</td><td scope="col" colspan="2" class="text-end pe-5">${currencyDisplay(arrangeFee)}đ</td></tr>` : ``;
    sumText += (plateNameFee != 0) ? `<tr><td scope="col" colspan="11" class="text-start ps-5">${plateNameFeeText}</td><td scope="col" colspan="2" class="text-end pe-5">${currencyDisplay(plateNameFee)}đ</td></tr>` : ``;
    sumText += (wellNameFee != 0) ? `<tr><td scope="col" colspan="11" class="text-start ps-5">${wellNameFeeText}</td><td scope="col" colspan="2" class="text-end pe-5">${currencyDisplay(wellNameFee)}đ</td></tr>` : ``;
    totalSurchanges = plateArrangeFee + plateNameFee + wellNameFee + packageBoxFee;
    subTotalAmount = totalAmount + totalSurchanges;
    sumText += `<tr><th scope="col" colspan="11" class="text-start ps-5">${subTotalLabel}</th><th scope="col" colspan="2" class="text-end pe-5 oligo-money">${currencyDisplay(subTotalAmount)}đ</th></tr>`;
    sumText += (f) ? `<tr>
        <td scope="col" colspan="13" class="text-start ps-5">Phụ phí ${currencyDisplay(lowNu)}đ đã được áp dụng với các trình tự có số nucleotide ít hơn ${stringLength[1]}</td>
    </tr>`: ``;
    sumText += (g) ? `<tr>
        <td scope="col" colspan="13" class="text-start ps-5">Phụ phí ${currencyDisplay(dryFee)}đ đã được áp dụng với các trình tự được yêu cầu sấy khô trước khi giao</td>
    </tr>`: ``;
    sumText += (h) ? `<tr>
        <td scope="col" colspan="13" class="text-start ps-5">Phụ phí ${currencyDisplay(normalizationFee)}đ đã được áp dụng với các trình tự có nồng độ tùy chỉnh</td>
    </tr>`: ``;
    sumText += `<tr>
        <td scope="col" colspan="13" rowspan="3" class="text-start ps-5">Các chuỗi nucleotide sau khi sản xuất theo quy trình tiêu chuẩn của Phù Sa genomics sẽ được sắp xếp và đóng gói vào các hộp trước khi giao hàng. Phù Sa genomics tạm tính quy cách đóng gói ${e} hộp ${d} cho ${productArr.length} chuỗi nucleotide đã nhập. Quy cách này có thể thay đổi cho phù hợp theo điều kiện thực tế.</td>
    </tr>`;
    sumText += `</table>`;
